        ...
        while (!salir) {
            ...
            switch (opcion) {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    Reportes(inventario,contadorProductos);
                    //
...
// termina el swt aqui
    ...
    // Generar todo los reportes aqui
    public  static  void Reportes(Producto[] inventario, int total) {
        ...
        switch (opcion) {
            case 1 :
                ReporteInventario(inventario,total, archivo);
                break;
            case 2 :
    ...
    public  static void ReporteInventario(Producto[] inventario,int contadorProductos, String timestamp) {
      String nombreArchivo = timestamp + "Inventario.pdf";
       try {
           PdfWriter writer = new PdfWriter(nombreArchivo);
           PdfDocument pdf = new PdfDocument(writer);
           Document document = new Document(pdf);

           document.add(new Paragraph("Reporte Inventario- TK--")

                   .setFontSize(16));
           document.add(new Paragraph("Fecha: " + java.time.LocalDate.now()));

           float[] columna = {3, 3, 2, 2, 2};
           Table table = new Table(columna);
           table.addHeaderCell("nNmbre");
           ...
           for(int i = 0; i < contadorProductos; i++ ) {
               if (inventario[i] != null ) {
                   table.addCell(inventario[i].nombre);
                   table.addCell(inventario[i].categoria);
                   table.addCell("Q" + String.format("///", inventario[i].precio));
                   table.addCell(String.valueOf(inventario[i].cantidad));
                   table.addCell(inventario[i].codigounico);
     ...
     public  static void ReporteVentas(String timestamp)  {
         String nombreArchivo = timestamp + "Veantas.pdf";
         File archivoVentas = new File("Ventas.txt");

         if (!archivoVentas.exists()) {
             System.out.println("No hay Ventas.");
             registrarBitacora("Generar_Reporte", "Error","No hay Ventas");
             return;
         }
         try {
             PdfWriter writer = new PdfWriter(nombreArchivo);
             ...
             document.add(new Paragraph("Reporte de Ventmas - TK-").setFontSize(16));
             document.add(new Paragraph("Fecha: "+ java.time.LocalDate.now()));

             float[] columna = {2, 2, 3, 2, 2, 2};
             Table table = new Table(columna);
             ...
             Scanner sc = new Scanner(archivoVentas);
             int Ventas = 0;
             double Ingresos = 0;
             while (sc.hasNextLine()) {
                 ...
                 if (datos.length >= 7) {
                     ...
                     table.addCell("Q" + datos[5]);
                     table.addCell("Q" + datos[6]);
                     Ventas++;
                     Ingresos += Double.parseDouble(datos[6]);

             }
         }
             sc.close();
             document.add(table);
             document.add(new Paragraph("TOTAL VENTAS: " + Ventas));
             document.add(new Paragraph("TOTAL INGRESOS: Q" + String.format(" ", Ingresos)));

             document.close();

             System.out.println("Reporte de ventas generado: " + nombreArchivo);
             registrarBitacora("GENERAR_REPORTE", "EXITO", "Reporte ventas: " + nombreArchivo);
     } catch (IOException e) {
             System.out.println("No se genero el Reporte. " + e.getMessage());
             registrarBitacora("GenerarReporte", "Error", "Error" + e.getMessage());
         }
